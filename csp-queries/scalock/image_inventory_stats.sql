...
select 'Number of containers' as Metric, count(DISTINCT id)::text as Value from containers where status = 'running'
UNION ALL
...
UNION ALL
select '% of images scanned and running' as Metric, TO_CHAR((SELECT count(DISTINCT oci_image_id) from containers)/(SELECT count(DISTINCT id) from registry_images)::real * 100,'fm990D00%')
