...
UNION ALL
select '% of images scanned but not used' as Metric, TO_CHAR((SELECT count(DISTINCT oci_image_id) from containers)/(SELECT count(DISTINCT id) from registry_images)::real * 100,'fm990D00%')
